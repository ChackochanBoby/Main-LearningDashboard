...
const ManageCoursePage = () => {
  ...
  const adminRole = location.pathname.startsWith("/admin")?"admin":"instructor"
  ...
  return (
    <main>
      <section id="manage-course-details" className="w-full p-8 bg-base-200">
        <div className="xl:container mx-auto grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4">
          <div>
            <figure className="h-72 aspect-[3/4] overflow-hidden mx-auto rounded-lg">
              <img
                src={courseDetails.thumbnail || "https://img.daisyui.com/images/stock/photo-1635805737707-575885ab0820.webp"}
          ...
          <div className="md:col-span-2 lg:col-span-3 text-center md:text-left flex flex-col items-center justify-between gap-5">
            ...
            <div className=" mx-auto md:ml-0">
              <span className="font-semibold text-lg">Price: ₹{courseDetails.price}</span>
            </div>
            {/* Buttons Section */}
            <div className="mt-4 flex flex-wrap gap-4 justify-center">
              <button onClick={()=>setCourseUpdateModal(true)} className="btn btn-secondary text-white">Update Course</button>
              <button onClick={()=>setThumbnailUpdateModal(true)} className="btn btn-primary text-white">Update Thumbnail</button>
              <Link to={`/${adminRole}/${courseDetails._id}/course-dashboard`} className="btn btn-info text-white">Course Content</Link>
              <button onClick={handleDeleteClick} className="btn btn-error text-white">Delete Course</button>
            </div>
          </div>
        </div>
        <Modal isOpen={isCourseUpdateModalOpen} modalControl={setCourseUpdateModal}>
            <UpdateCourseDetailsForm courseId={courseDetails._id}/>
        </Modal>
        <Modal isOpen={isThumbnailUpdateModalOpen} modalControl={setThumbnailUpdateModal}>
